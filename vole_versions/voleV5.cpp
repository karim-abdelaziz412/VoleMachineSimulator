...
public:
    ...
    void loadProgram(const vector<string>& program) {
        ...
        for (size_t i = 0; i < program.size(); ++i) {
            if (memIndex >= memorySize - 1) {  
                throw runtime_error("Program exceeds memory capacity.");
            }
            ...
            try {
                ...
                throw runtime_error("Invalid program instruction at line " + to_string(i) + ": Not a valid hexadecimal number.");
            } catch (const out_of_range&) {
                throw runtime_error("Program instruction at line " + to_string(i) + " exceeds memory byte range.");
            }
        }
    }
    ...
    void displayScreen() const {
        cout << "Memory (16x16 matrix representation):" << endl;
        for (int i = 0; i < memorySize; i += 16) {
            cout << "0x" << hex << setw(2) << setfill('0') << i << ": ";
            for (int j = 0; j < 16; ++j) {
                cout << setw(2) << setfill('0') << static_cast<int>(mem[i + j]) << " ";
