...
public:
    ...
    void loadProgram(const vector<string>& program) {
        ...
        for (size_t i = 0; i < program.size(); ++i) {
            if (memIndex + 1 >= memorySize) {
                throw runtime_error("Program exceeds memory bounds.");
            }
            ...
            try {
                ...
                throw runtime_error("Invalid instruction at line " + to_string(i) + ": Not a valid hexadecimal number.");
            } catch (const out_of_range&) {
                throw runtime_error("Program instruction at line " + to_string(i) + " exceeds memory range");
            }
        }

        if (memIndex > memorySize) {
            throw runtime_error("The program exceeds the available memory space.");
        }
...
public:
    ...
    void fetch() {
        cout << "Program Counter before fetch: " << cu.getProgramCounter() << endl;
    ...
    void execute() {
        ...
        switch (operationcode) {
            ...
            case 0x5:
            {
                float val1 = static_cast<float>(reg.get(operand1));
                float val2 = static_cast<float>(reg.get(operand2));
                float result = alu.addFloat(val1, val2);
            ...
            case 0xB:
                ...
                break;
            case 0xC:
                cout << "HALT instruction executed. Program will stop." << endl;
...
public:
    ...
    void loadProgramFromFile() {
        ...
        cout << "Program loaded successfully from " << filename << endl;
    }
